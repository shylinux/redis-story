Volcanos(chat.ONIMPORT, {
	_init: function(can, msg) { can.require(["/plugin/story/studiolayout.js"], function() {
		can.onimport.project(can, msg, aaa.SESS, function(event, sess, value) { var content, profile, display; return {
			display: {index: "web.code.redis.shells", args: sess, _init: function(sub) { display = sub }},
			content: {index: "web.code.redis.keys", args: sess},
			profile: can.isCmdMode() && {index: "web.code.redis.commands", args: sess, _init: function(sub) {
				sub.onexport.output = function(sub) {
					sub.onaction.helpCmd = function(event, can) { var xterm = display._plugins[0].sub, target = event.target
